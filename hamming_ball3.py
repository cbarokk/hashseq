...
def get_minutes(seqences):
    ...
    events = np.empty([len(sequences), len(sequences[0])])
...
def get_dist(seqences):
    data = []
    for seq in sequences:
        x=[]
        y=[]
        for e in seq:
            x.append(minute_of_the_week(int(e[0])))
            y.append(int(e[1]))
        data.append((x,y))
    return data


def get_elapsed(seqences):
    elapsed = np.empty([len(sequences), len(sequences[0])-1])
    
    for i, seq in enumerate(sequences):
        for j in range(len(seq)-1):
            elapsed[i][j] = int(seq[j+1][0]) - int(seq[j][0])
    return elapsed
...
def plot_dist(streams, key):
    plt.title(key)
    #plt.xlim((0, 51))
    #max_y = np.max(streams)
    #plt.ylim((0, max_y))
    print "found" , len(streams)

    colors = cm.rainbow(np.linspace(0, 1, len(streams)))
