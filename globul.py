...
def load_data():
    ...
    #groups =  store.select('data', columns=['callingSubscriberIMSI', 'cell_ID'], where=where, sort=False).groupby(['callingSubscriberIMSI'])
    #groups =  store.select('data', columns=['callingSubscriberIMSI', 'cell_ID'], where=where).groupby(['callingSubscriberIMSI'], sort=False).apply(lambda x: x.sort_index())
